   ...
   async iniciarJuego(urlGuion) {
    ...
    renderizar() {
        ...
        if (this.escenaActual.hasOwnProperty('imagenFondo')) {
            this.fondoActual = this.escenaActual.imagenFondo; // Guardar estado
            this.elementosDOM.fondo.style.backgroundImage = `url('${this.fondoActual}')`;
        }
        
        // --- LÓGICA DE PERSONAJES MÚLTIPLES ---
        this.renderizarPersonajes();
        // --- MEJORA: LLAMADA A LA MÁQUINA DE ESCRIBIR ---
        // En lugar de asignar el texto directamente:
        // textoDialogo.textContent = this.escenaActual.texto;
        ...
        // --- MODIFICACIÓN: Renderizado de Opciones Condicionales ---
        if (this.escenaActual.tipo === 'decision') {
            cuadroDialogo.classList.add('decision');
            this.escenaActual.opciones.forEach(opcion => {
                // Si la opción tiene una condición, la evaluamos primero
                if (opcion.condicion && !this.evaluarCondicion(opcion.condicion)) {
                    return; // Si la condición no se cumple, no se muestra el botón
                }
                const boton = document.createElement('button');
                boton.textContent = opcion.texto;
                boton.addEventListener('click', () => {
                    // Procesamos los efectos de la opción ANTES de avanzar
    ...
    renderizarPersonajes() {
        ...
        personajesEnEscena.forEach(personajeData => {
            ...
            if (personajeData.nombre === hablante) {
                ...
                spriteFinal = personajeData.sprite.replace(/_([^_]+)\.png$/, '_hablando.png');
